# code by Yayan XD
# my facebook ( https://www.facebook.com/mr.love1you )
...
user = []
loop = 0
url_mb =https://www.facebook.com/mr.love1you"
url_ip = ""
url_graph = "/{}"
header_grup = {"user-agent": "Ovi Raj/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]"}
...
def loading():
    ...
    for i in range(50):
        time.sleep(0.1)
        sys.stdout.write(f"\r {O}[{V}•{I}] {H}Loading...{N} " + animation[i % len(animation)] +"\x1b[0m ")
        sys.stdout.flush()
    print("")
# LOGO
def logo():
...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        print(f' %s[%s+%s] Number of Accounts OK : %s%s%s'%(O,V,I,str(len(ok)), OVI))
        print(f' [%s+%s] Number of Accounts CP : %s%s%s'%(O,V,I,str(len(cp)),OVI))
        cek_cp = input(f"{H}══════════════════════════════════════════\n [{H}+{H}] Show CP detector options [{H}Y{N}/{M}t{N}]: ")
        if cek_cp =="":
            print(f"\n [{M}!{N}] Don't be empty");hasil(ok,cp)
        elif cek_cp in["Y","y"]:
            jalan(f" {O}[{V}!{I}] Play airplanemode first");time.sleep(5)
            ww=input(f"\n {N}[{K}?{N}] Change password when {BM}TAP YES{N} [{H}Y{N}/{M}t{N}]: ")
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {O}[{V}•{I}] Password example : {H}james123{N}")
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {O}[{V}?{I}] Account : {K}{kontol.replace("[JAMES-CP] ", "")}{N}')
                try:
            ...
            jalan(f"\n {O}[{V}•{I}] {N}Ok, thank you. Retrun SC type '{H}python UserCrack.py{N}'");exit()
        else:
            print(f"\n {O}[{V}!{I}] Choose Y/t");hasil(ok,cp)
    else:
...
class __chigoue__:
    ...
    def chi(self, id):
        ...
        if crot in[""]:
            print(f" {O}[{V}×{I}] Don't be empty");__chigoue__().chi(id)
        elif crot in["Y","y"]:
            Apk.append("y")
        elif crot in["T","t"]:
            Apk.append("t")
...
# PROSES CRACK METODE 3 in 1
    ...
    def kombinasi_pw(self,url):
        ...
        if pw in[""]:
            ...
            print(f"\n {O}[{V}!{I}] Correct input");self.kombinasi_pw(url)
